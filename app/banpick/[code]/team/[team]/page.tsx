...
export default function TeamBanpickPage() {
    ...
    sub = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            payload.new.progress?.decider &&
            payload.new.progress?.decider !== lastDecider
          ) {
  ...
  if (authError) {
    return <div className="flex min-h-screen items-center justify-center text-red-600">无效的队伍链接</div>;
  }
  if (!room || !team) {
  ...
  async function handleAction(map: string) {
    ...
    if (newProgress.currentStep >= steps.length) {
      ...
      newProgress.decider = map_pool.find(
    ...
    if (error) {
      ...
      toast.success(
        `${currentStep.action === "ban" ? "Ban" : "Pick"} ${map} 成功！`
      );
    }
  }
    ...
    <div className="flex min-h-screen flex-col items-center gap-8 p-4 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700">
      <Card className="w-full max-w-2xl p-8 flex flex-col gap-6 bg-white/80 dark:bg-black/60 shadow-2xl">
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm font-mono bg-muted px-2 py-1 rounded">{team === "A" ? "你的队伍" : "对手队伍"}</span>
